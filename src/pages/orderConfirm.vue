...
export default {
  ...
  methods:{
    async getAddressList(){
      let data = await orderService.getAddressList()
      this.list = data.list;
    },
    // 打开新增地址弹框
    openAddressModal(){
    ...
    async submitAddress(){
      ...
      await orderService.submitAddress(url, method, params)
      this.closeModal();
      this.getAddressList();
    ...
    orderSubmit(){
      ...
      orderService.orderSubmit({shippingId:item.id}).then((res)=>{
        this.$router.push({
          path:'/order/pay',
          query:{
            orderNo:res.orderNo
          ...
          .addr-list{
            ...
            .addr-add{
              ...
              .icon-add{
                ...
                height:30px;
                border-radius:50%;
                background:url('../assets/images/icon-add.png') #E0E0E0 no-repeat center;
